  ...
  <Teleport to="body">
    ...
    >
      <a-flex align="end" class="mb-large">
        ...
        <a-tag class="bold" color="grey">{{ kanagataData.id }}</a-tag>
      </a-flex>
      <a-flex
        vertical
        gap="small"
        v-if="kanagataData.controlledSparepart.length"
      >
        <a-card
          :bordered="false"
          size="small"
          v-for="(data, index) in kanagataData.controlledSparepart"
          :key="index"
        >
          <a-descriptions class="compact-desc" bordered :column="2">
            <a-descriptions-item label="Name" class="ellipsis-text" :span="2">{{
              data.sparepartName
            }}</a-descriptions-item>
...
const { detailKanagata: kanagataData } = storeToRefs(dashboardStore)
...
const handleAction = async (action, data) => {
  ...
  if (action === 'end') {
    await sparepartControlledStore.update(
      stockId,
      kanagataData.value.id,
      'kanagata',
      { finishedAt: dayjs() },
    )
    await dashboardStore.getDetail(kanagataData.value.id)
  }
  if (action === 'remove') {
...
watch(
  () => dashboardStore.drawer.kanagataId,
  async newId => {
    if (!newId) return
